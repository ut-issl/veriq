...
def test_table_verification_evaluation_stores_leaf_paths() -> None:
    ...
    assert results.get_value(nominal_path) is True
    assert results.get_value(safe_path) is False
...
def test_bool_and_table_verifications_coexist() -> None:
    ...
    assert results.get_value(bool_path) is True
...
def test_multidim_table_verification_evaluation() -> None:
    ...
    assert results.get_value(make_path("initial", "nominal")) is True
    assert results.get_value(make_path("initial", "safe")) is False
    assert results.get_value(make_path("cruise", "nominal")) is True
    assert results.get_value(make_path("cruise", "safe")) is True
